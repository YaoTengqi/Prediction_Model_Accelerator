...
entity concat_concat_addr_s_axi is
generic (
...
end entity concat_concat_addr_s_axi;
...
architecture behave of concat_concat_addr_s_axi is
    ...
    signal int_concat_flag     : UNSIGNED(0 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_CONCAT_FLAG_DATA_0 =>
                        rdata_data <= RESIZE(int_concat_flag(0 downto 0), 32);
                    when ADDR_CONCAT_FLAG_CTRL =>
                        rdata_data(0) <= int_concat_flag_ap_vld;
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_concat_flag <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (concat_flag_ap_vld = '1') then
                    int_concat_flag <= UNSIGNED(concat_flag);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_concat_flag_ap_vld <= '0';
            elsif (ACLK_EN = '1') then
                if (concat_flag_ap_vld = '1') then
                    int_concat_flag_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_CONCAT_FLAG_CTRL) then
                    int_concat_flag_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
