...
// ==============================================================
`timescale 1ns/1ps
module reshape_reshape_addr_s_axi
#(parameter
    ...
    input  wire [0:0]                    reshape_flag,
    input  wire                          reshape_flag_ap_vld,
...
localparam
    ...
    ADDR_IER                    = 7'h08,
    ADDR_ISR                    = 7'h0c,
    ADDR_INPUT_DATA_ADDR_DATA_0 = 7'h10,
    ADDR_INPUT_DATA_ADDR_CTRL   = 7'h14,
    ADDR_INPUTS_DATA_0          = 7'h18,
    ADDR_INPUTS_DATA_1          = 7'h1c,
    ADDR_INPUTS_CTRL            = 7'h20,
    ADDR_OUTPUTS_DATA_0         = 7'h24,
    ADDR_OUTPUTS_DATA_1         = 7'h28,
    ADDR_OUTPUTS_CTRL           = 7'h2c,
    ADDR_ROWS_DATA_0            = 7'h30,
    ADDR_ROWS_CTRL              = 7'h34,
    ADDR_COLS_DATA_0            = 7'h38,
    ADDR_COLS_CTRL              = 7'h3c,
    ADDR_RESHAPE_FLAG_DATA_0    = 7'h40,
    ADDR_RESHAPE_FLAG_CTRL      = 7'h44,
    WRIDLE                      = 2'd0,
    WRDATA                      = 2'd1,
    WRRESP                      = 2'd2,
    WRRESET                     = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg                           int_reshape_flag_ap_vld;
    reg  [0:0]                    int_reshape_flag = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_RESHAPE_FLAG_DATA_0: begin
                    rdata <= int_reshape_flag[0:0];
                end
                ADDR_RESHAPE_FLAG_CTRL: begin
                    rdata[0] <= int_reshape_flag_ap_vld;
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (reshape_flag_ap_vld)
            int_reshape_flag <= reshape_flag;
    end
end

// int_reshape_flag_ap_vld
always @(posedge ACLK) begin
    if (ARESET)
        int_reshape_flag_ap_vld <= 1'b0;
    else if (ACLK_EN) begin
        if (reshape_flag_ap_vld)
            int_reshape_flag_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_RESHAPE_FLAG_CTRL)
            int_reshape_flag_ap_vld <= 1'b0; // clear on read
    end
end
