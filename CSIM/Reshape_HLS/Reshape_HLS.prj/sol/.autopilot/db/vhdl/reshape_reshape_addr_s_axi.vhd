...
entity reshape_reshape_addr_s_axi is
generic (
...
port (
    ...
    reshape_flag          :in   STD_LOGIC_VECTOR(0 downto 0);
    reshape_flag_ap_vld   :in   STD_LOGIC;
    ap_start              :out  STD_LOGIC;
    ap_done               :in   STD_LOGIC;
    ap_ready              :in   STD_LOGIC;
    ap_idle               :in   STD_LOGIC
);
end entity reshape_reshape_addr_s_axi;
...
architecture behave of reshape_reshape_addr_s_axi is
    ...
    constant ADDR_INPUTS_DATA_0          : INTEGER := 16#18#;
    constant ADDR_INPUTS_DATA_1          : INTEGER := 16#1c#;
    constant ADDR_INPUTS_CTRL            : INTEGER := 16#20#;
    constant ADDR_OUTPUTS_DATA_0         : INTEGER := 16#24#;
    constant ADDR_OUTPUTS_DATA_1         : INTEGER := 16#28#;
    constant ADDR_OUTPUTS_CTRL           : INTEGER := 16#2c#;
    ...
    constant ADDR_RESHAPE_FLAG_DATA_0    : INTEGER := 16#40#;
    constant ADDR_RESHAPE_FLAG_CTRL      : INTEGER := 16#44#;
    ...
    signal int_reshape_flag_ap_vld : STD_LOGIC;
    signal int_reshape_flag    : UNSIGNED(0 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_RESHAPE_FLAG_DATA_0 =>
                        rdata_data <= RESIZE(int_reshape_flag(0 downto 0), 32);
                    when ADDR_RESHAPE_FLAG_CTRL =>
                        rdata_data(0) <= int_reshape_flag_ap_vld;
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_reshape_flag <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (reshape_flag_ap_vld = '1') then
                    int_reshape_flag <= UNSIGNED(reshape_flag);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_reshape_flag_ap_vld <= '0';
            elsif (ACLK_EN = '1') then
                if (reshape_flag_ap_vld = '1') then
                    int_reshape_flag_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_RESHAPE_FLAG_CTRL) then
                    int_reshape_flag_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
