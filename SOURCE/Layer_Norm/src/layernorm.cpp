...
#pragma HLS INTERFACE mode = m_axi port = inputs bundle = ln_data latency = 32          \
    num_write_outstanding = 1 num_read_outstanding = 64 max_write_burst_length = 64     \
    max_read_burst_length = 64 depth = M_AXI_DEPTH
#pragma HLS INTERFACE mode = m_axi port = outputs bundle = ln_data latency = 32         \
    num_write_outstanding = 1 num_read_outstanding = 64 max_write_burst_length = 64     \
    max_read_burst_length = 64 depth = M_AXI_DEPTH
#pragma HLS INTERFACE mode = m_axi port = paras bundle = ln_paras latency = 32          \
    num_write_outstanding = 1 num_read_outstanding = 64 max_write_burst_length = 64     \
    max_read_burst_length = 64 depth = M_AXI_DEPTH
...
#pragma HLS STREAM variable=mean_a depth=64
    hls::stream<typename WideType<t_InpDataType, 1>::t_TypeInt> mean_b;
#pragma HLS STREAM variable=mean_b depth=64
    hls::stream<typename WideType<t_ParaType, 1>::t_TypeInt> stddev;
#pragma HLS STREAM variable=stddev depth=64
    hls::stream<typename WideType<t_ParaType, nPE>::t_TypeInt> norm;
#pragma HLS STREAM variable=norm depth=16
    hls::stream<typename WideType<t_OutDataType, nPE>::t_TypeInt> relu;
#pragma HLS STREAM variable=relu depth=16
...
#pragma HLS DATAFLOW
    ...
    DataMover_B<t_ParaType, t_AXI_DataType, nPE_LOG>(cols_log, rows, gama_addr, beta_addr, paras, gamma, beta);
    Mean<t_InpDataType, t_MeanSumType, nPE_LOG>(cols_log, rows, data_copy_a, mean_a, mean_b);
    StdDev<t_InpDataType, t_ParaType, t_StdDiffType, t_StdSumType, nPE_LOG>(cols_log, rows, data_copy_b, mean_a, stddev);
    Norm<t_InpDataType, t_ParaType, nPE_LOG>(cols_log, rows, data_copy_c, mean_b, stddev, gamma, beta ,norm);
    Relu<t_ParaType, t_OutDataType, nPE_LOG>(cols_log, rows, norm, relu);
    Store<t_OutDataType, t_AXI_DataType, nPE_LOG>(cols_log, rows, output_data_addr, outputs, relu);
}
